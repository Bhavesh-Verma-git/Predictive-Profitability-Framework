...
baseline_labels = pd.read_parquet(os.path.join(exp_dir, "data", "pseudo_labels.parquet"))
...
def calculate_top_100k(R_cross_sell=None, modify_ecl=False, dynamic_urr=False):
    ...
    if modify_ecl:
        C_ecl = ((f1 * f11 * LGD) - (f4 * 0.01)) + (f3 * COLL) + (f3 * f1 * 1.0)
    else:
    ...
    return set(df.loc[top_100k, 'id'])

# Baseline
baseline_ids = set(baseline_labels[baseline_labels['label'] == 1]['id'])
print(f"Baseline Top 100k Size: {len(baseline_ids)}")
print("-" * 50)

# Exp 1: Add R_cross_sell = f4 * 0.002
print("Running Exp 1 (Cross-sell: R_cross_sell = f4 * 0.002)...")
exp1_ids = calculate_top_100k(R_cross_sell=f4 * 0.002)
overlap_1 = len(exp1_ids.intersection(baseline_ids))
print(f"Exp 1 Overlap: {overlap_1 / 100000 * 100:.2f}% ({overlap_1} customers)")
print(f"New Customers Added: {100000 - overlap_1}")
print("-" * 50)

# Exp 2: C_ecl = (f1 * f11 * LGD) - (f4 * 0.01)
print("Running Exp 2 (Risk Mitigator: C_ecl = (f1*f11*LGD) - (f4*0.01))...")
exp2_ids = calculate_top_100k(modify_ecl=True)
overlap_2 = len(exp2_ids.intersection(baseline_ids))
print(f"Exp 2 Overlap: {overlap_2 / 100000 * 100:.2f}% ({overlap_2} customers)")
print(f"New Customers Added: {100000 - overlap_2}")
print("-" * 50)

# Exp 3: dynamic_urr = np.where(f21 > 0, 0.98, 0.75)
print("Running Exp 3 (Dynamic URR: active=0.98, inactive=0.75)...")
exp3_ids = calculate_top_100k(dynamic_urr=True)
overlap_3 = len(exp3_ids.intersection(baseline_ids))
print(f"Exp 3 Overlap: {overlap_3 / 100000 * 100:.2f}% ({overlap_3} customers)")
print(f"New Customers Added: {100000 - overlap_3}")
print("-" * 50)

# Exp 4: Combo 1 & 3
print("Running Exp 4 (Combo 1 & 3: Cross-sell + Dynamic URR)...")
exp4_ids = calculate_top_100k(R_cross_sell=f4 * 0.002, dynamic_urr=True)
overlap_4 = len(exp4_ids.intersection(baseline_ids))
print(f"Exp 4 Overlap: {overlap_4 / 100000 * 100:.2f}% ({overlap_4} customers)")
print(f"New Customers Added: {100000 - overlap_4}")
print("-" * 50)

# Exp 5: Combo 2 & 3
print("Running Exp 5 (Combo 2 & 3: Risk Mitigator + Dynamic URR)...")
exp5_ids = calculate_top_100k(modify_ecl=True, dynamic_urr=True)
overlap_5 = len(exp5_ids.intersection(baseline_ids))
print(f"Exp 5 Overlap: {overlap_5 / 100000 * 100:.2f}% ({overlap_5} customers)")
print(f"New Customers Added: {100000 - overlap_5}")
print("-" * 50)
